...
export const BillingHistoryTable = ({ history }: BillingHistoryTableProps) => {
  return (
    <div className="bg-[#121814] border border-[#1b251e] rounded-xl overflow-hidden">
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="border-b border-[#1b251e] bg-[#0a0e0c]/50">
              ...
              <th className="py-4 px-6 text-xs font-semibold text-zinc-400 uppercase tracking-wider">Amount</th>
          ...
          <tbody className="divide-y divide-[#1b251e]">
            {history.map((item) => (
              <tr key={item.id} className="hover:bg-zinc-800/20 transition-colors">
                <td className="py-4 px-6 text-sm text-zinc-300 font-medium whitespace-nowrap">{item.date}</td>
                <td className="py-4 px-6 text-sm text-zinc-400">{item.description}</td>
                ...
                <td className="py-4 px-6 text-right whitespace-nowrap">
                  <button className="text-zinc-500 hover:text-emerald-500 transition-colors p-2 rounded hover:bg-emerald-500/10 inline-flex" title="Download Invoice">
                    <Download size={16} />
                  </button>
                </td>
              </tr>
